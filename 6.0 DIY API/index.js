...
app.get("/jokes/:id", (req, res) => {
  ...
  const found = jokes.find((i) => i.id === id)
  res.json(found);
})
...
app.get("/filter", (req, res) => {
  ...
  const found = jokes.filter((i) => i.jokeType === type)
  res.json(found);
})
...
app.put("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const found = jokes.findIndex((i) => i.id === id)
  const joke = {
    id: jokes.length + 1,
    jokeText: req.body.text,
    jokeType: req.body.type,
...
app.delete("/jokes/:id", (req, res) => {
  ...
  const found = jokes.findIndex((i) => i.id === id)
  if (found > -1) {
    jokes.splice(found, 1);
    res.json("Deleted!");
  } else {
    res.status(404).send({ Error: "Dit me may deo dua!"});
  }
})
